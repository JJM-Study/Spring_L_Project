...
<!DOCTYPE html>
    ...
    <ul>
        ...
        <li>
            ...
            <div id="div-best" class="px-2">
             <%-- 라이브러리 사용 가능성, SEO, 스크린 리더 등 감안. --%>
                <ul id="ul-best" class="row list-unstyled g-2">
                    <c:forEach var="item" items="${bestList}">
                        <li class="col-6 col-md-1 mb-2">
                              ...
                              <div class="card-body">
                                ...
                                <c:choose>
                                    ...
                                    <c:when test="${item.isInCart==false}">
                                        <button class="btn btn-primary btn-sm w-100 add-to-cart-btn" data-prodno="${item.prodNo}">카트 담기</button>
                                    <input type="hidden" class="qty" value="1" />
                                    </c:when>
                                    <c:otherwise>
                                        <button class="btn btn-primary btn-sm w-100 in-cart-btn" data-prodno="${item.prodNo}" onclick="location.href='/cart/cartlist'">카트에 있음</button>
                                    </c:otherwise>
                                </c:choose>
                              </div>
                            </div>
